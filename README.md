...
- Convenient batch files, script files and makefiles are included:
  ...
  - The batch files and scripts for building DOS executables are a convenience: the make files can be used by calling

    `make -f`

    directly with the appropriate file name.

  - makew32.bat calls lazbuild to build config.lpi with the '--bm="Release Win32"' parameter to produce an executable file for 32-bit Windows.

  - makew64.bat calls lazbuild to build config.lpi with the '--bm="Release Win64"' parameter to produce an executable file for 64-bit Windows.

  - makel64.sh calls lazbuild to build config.lpi with the '--bm="Release Linux x64"' parameter to produce an executable file for 64-bit Linux.

  - makel32.sh calls lazbuild to build config.lpi with the '--bm="Release Linux x32"' parameter to produce an executable file for 32-bit Linux.

  - lazbuild can be called manually using the above parameters.
...
- A Lazarus project is also included, it contains convenient build modes for compiling DOS targets (real and protected mode), Windows and Linux targets for 32 or 64-bit architectures; editing the sources, and debugging the non-DOS exclusive parts of the program easily under Windows or Linux.
